...
def get_user_tasks(db: Session, user_id: str, skip: int = 0, limit: int = 100, search: Optional[str] = None):
    # Get tasks where user is owner OR user is a collaborator
    shared_task_ids = db.query(TaskShare.task_id).filter(TaskShare.user_id == user_id).all()
    shared_task_ids = [tid[0] for tid in shared_task_ids]
