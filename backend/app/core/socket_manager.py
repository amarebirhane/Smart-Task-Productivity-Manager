...
class SocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str):
        ...
        logger.info(f"User {user_id} connected via WS. Total connections for user: {len(self.active_connections[user_id])}")
    ...
    async def send_personal_message(self, message: dict, user_id: str):
        """Send a message to all active tabs of a specific user."""
        if user_id in self.active_connections:
